...
# `config_email.py` se mantiene fuera de Git (ver `.gitignore`) porque puede contener credenciales.
# Para que el proyecto funcione en un clon limpio, soportamos configuración por variables de entorno.
try:
    import config_email  # type: ignore
except Exception:
    config_email = None
...
def _get_setting(key: str, default: Any = None) -> Any:
    if config_email is not None and hasattr(config_email, key):
        return getattr(config_email, key)
    env_value = os.getenv(key)
    return default if env_value is None or str(env_value).strip() == "" else env_value


# Configuración de Flask-Mail (config_email.py o variables de entorno)
app.config["MAIL_SERVER"] = _get_setting("MAIL_SERVER", "smtp.gmail.com")
app.config["MAIL_PORT"] = int(_get_setting("MAIL_PORT", 587))
app.config["MAIL_USE_TLS"] = _env_bool(str(_get_setting("MAIL_USE_TLS", "true")))
app.config["MAIL_USERNAME"] = _get_setting("MAIL_USERNAME", "")
app.config["MAIL_PASSWORD"] = _get_setting("MAIL_PASSWORD", "")
app.config["MAIL_DEFAULT_SENDER"] = _get_setting("MAIL_DEFAULT_SENDER", "")
app.config["PROJECT_NAME"] = _get_setting("PROJECT_NAME", "NACHOHER")
