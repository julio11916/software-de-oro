...
# -*- coding: utf-8 -*-
"""
Integración robusta del backup a la BD actual.
Maneja formato binario, extrae datos inteligentemente y resuelve conflictos.
"""
...
def es_formato_custom(ruta):
    """Verificar si es formato custom de PostgreSQL"""
    with open(ruta, 'rb') as f:
        return f.read(4) == b'PGDM'

def importar_con_pg_restore(ruta):
    """Importar usando pg_restore para formato custom"""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️  pg_restore con advertencias: {result.stderr[:200]}")
            return True  # Intentar continuar aunque haya advertencias
            
    except FileNotFoundError:
        print("❌ pg_restore no encontrado")
        return False
    except Exception as e:
        print(f"❌ Error en pg_restore: {e}")
        return False

def importar_con_psql(ruta):
    """Importar usando psql para archivos SQL"""
    ...
    try:
        cmd = [
            ...
            '--dbname', 'software_de_oro',
            '--file', ruta,
            '-v', 'ON_ERROR_STOP=0'  # Continuar aunque haya errores
        ]
        ...
        print("✅ Importación completada (errores ignorados si es necesario)")
        return True
...
def arreglar_integridad(conn):
    """Arreglar problemas de integridad encontrados"""
    print("\n🔧 Arreglando integridad de datos...")
    cursor = conn.cursor()
    
    try:
        # 1. Verificar y arreglar referencias inválidas en detalle_pedido
        ...
        if deleted > 0:
            print(f"   ✅ Eliminados {deleted} detalles huérfanos")
        
        # 2. Arreglar referencias en pagos
        ...
        if updated > 0:
            print(f"   ✅ Actualizados {updated} pagos inválidos")
        
        # 3. Arreglar fechas NULL en pedidos
        ...
        conn.rollback()
        print(f"⚠️  Error menor en integridad (continuando): {e}")
        return True

def mostrar_resumen_final(conn):
    """Mostrar resumen de datos después de la integración"""
    print("\n" + "="*70)
